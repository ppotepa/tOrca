...
use crate::pairing_rules::normalize_pairing_item;
use crate::{
    ChatMessage, ClientRuntime, ContactRecord, ConversationSummary, InviteCode, MessageState,
    PairingItem, ReceiptSendEffect, RuntimeClock, RuntimeError, RuntimeIdentity, RuntimeProfile,
    RuntimeRequest, RuntimeResult, RuntimeStorage, RuntimeTorStatus, RuntimeTransport,
};
...
struct MemoryStorage {
    ...
    pending_receipts: Vec<ReceiptSendEffect>,
}
...
impl RuntimeStorage for MemoryStorage {
    ...
    fn pending_receipts(&self) -> RuntimeResult<Vec<ReceiptSendEffect>> {
        Ok(self.pending_receipts.clone())
    }
}
...
mod tests {
    ...
    fn c_api_conforms_to_shared_runtime_message_and_event_flow() {
        ...
        let bootstrap = bootstrap_runtime(runtime);
        assert_eq!(bootstrap["ok"], true);
        let bootstrap_again = bootstrap_runtime(runtime);
        ...
        let forwarded = apply_message_transport_outcome(runtime, message_id, crate::MessageTransportOutcome::Forwarded);
        assert_eq!(forwarded["result"]["state"], "SENT");
        let delivered = apply_message_transport_outcome(runtime, message_id, crate::MessageTransportOutcome::Delivered);
    ...
    fn bootstrap_runtime(runtime: *mut TorchatClientRuntime) -> serde_json::Value {
        let runtime = unsafe { &mut *runtime };
        let response = crate::RuntimeResponse::ok(None, runtime.runtime.bootstrap_runtime().unwrap())
            .expect("bootstrap response");
        serde_json::to_value(response).unwrap()
    }

    fn apply_message_transport_outcome(
        runtime: *mut TorchatClientRuntime,
        message_id: &str,
        outcome: crate::MessageTransportOutcome,
    ) -> serde_json::Value {
        let runtime = unsafe { &mut *runtime };
        let response = match runtime.runtime.apply_message_transport_outcome(
            uuid::Uuid::parse_str(message_id).unwrap(),
            outcome,
        ) {
            Ok(result) => crate::RuntimeResponse::ok(None, result).expect("transport response"),
            Err(error) => crate::RuntimeResponse::error(None, error),
        };
        serde_json::to_value(response).unwrap()
    }
