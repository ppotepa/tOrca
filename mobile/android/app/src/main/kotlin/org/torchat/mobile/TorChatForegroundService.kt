...
class TorChatForegroundService : Service() {
                    ...
                    while (initialProfile == null && isActive) {
                        ...
                        try {
                            initialProfile = withContext(Dispatchers.IO) {
                                ...
                                if (BuildConfig.DEBUG && BuildConfig.TORCHAT_DEV_PAIR && BuildConfig.TORCHAT_DEV_PROFILE.isNotBlank()) {
                    ...
                    scope.launch {
                        while (isActive) {
                            ...
                            runCatching { controller.syncPairingInbox() }
                                .onSuccess { publish(mapOf("type" to "chat_state_changed")) }
                                .onFailure { error -> Log.w("TorChat-Runtime", "Pairing inbox refresh failed", error) }
                        }
                    }
                    scope.launch {
                        var retrySeconds = 1L
    ...
    companion object {
        ...
        fun identityInfo(): Map<String, Any?>? = activeIdentity?.let {
            mapOf("installationId" to it.installationId(), "fingerprint" to it.fingerprint())
        }
    }
}
