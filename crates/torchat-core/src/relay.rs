...
impl RelayPayloadV1 {
    ...
    pub fn decode(value: &str) -> Result<Self, String> {
        ...
        let version = match &payload {
            Self::Welcome { version, .. } | Self::Application { version, .. } => *version,
        };
    ...
    pub fn verify_welcome(
        ...
        if !verify_signature(
            &sender.public_key,
            &welcome_signing_bytes(sender, recipient, welcome, ratchet_tree),
            signature,
        ) {
    ...
    pub fn decode_welcome(&self) -> Result<(Vec<u8>, Vec<u8>), String> {
        let Self::Welcome {
            welcome,
            ratchet_tree,
            ..
...
mod tests {
    ...
    fn signed_welcome_round_trip_rejects_tampering() {
        ...
        let payload =
            RelayPayloadV1::welcome(&alice, "Alice", bob.installation_id(), b"welcome", b"tree");
