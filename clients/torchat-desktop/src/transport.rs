use crate::model::{
    ChallengeResponse, ClientFrame, DirectoryEntry, ProfileResponse, RegisterRequest, ServerFrame,
    SessionResponse,
};
...
pub enum RelayEvent {
    ...
    Connected,
    SearchResults(Vec<DirectoryEntry>),
...
impl ApiTransport {
    ...
    async fn search_directory(&self, token: &str, query: &str) -> Result<Vec<DirectoryEntry>> {
        let mut url = self.base_url.join("/v1/directory/search")?;
                ...
                tokio::select! {
                    command = command_rx.recv() => {
                        match command {
                            ...
                            Some(RelayCommand::Search(query)) => {
                                match transport.search_directory(&token, &query).await {
                                    Ok(results) => { let _ = event_tx.send(RelayEvent::SearchResults(results)); }
                                    Err(error) => { let _ = event_tx.send(RelayEvent::Error(format!("search failed: {error:#}"))); }
                                }
                            }
