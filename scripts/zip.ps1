...
function New-PlaceholderDiagnostic {
    param(
        [Parameter(Mandatory = $true)][string]$Path,
    ...
    $message = @"
Placeholder diagnostic artifact.
This file was generated by the snapshot script because requested diagnostics
could not be collected from the device/platform:

$Reason
"@
    Write-Utf8 -Path $Path -Value $message
}
...
try {
    ...
    try {
        ...
        $androidLogCaptured = $false
        if (Test-Path -LiteralPath $androidLog) {
            ...
            if (-not $androidContents -or $androidContents -match '^No connected ADB device found') {
                ...
                $androidLogCaptured = $true
            }
        }

        if (-not $androidLogCaptured) {
            $allowMissingReason = 'No Android process diagnostics were available for the latest run (no device connected or no logs captured).'
            if (-not $AllowMissingAndroid) {
                Write-Warning "Android logs are incomplete. $allowMissingReason`nA placeholder log entry was created at $androidLog."
            }
            New-PlaceholderDiagnostic -Path $androidLog -Reason $allowMissingReason
        }
        ...
        try {
            ...
            if (Test-Path -LiteralPath (Join-Path $collectorRoot 'diagnostics\android\android-app.log')) {
