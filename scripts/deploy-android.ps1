...
$runArgs = @{ DeviceAddress = $DeviceAddress; ResetDevState = $ResetDevState; Clean = $Clean }
& (Join-Path $PSScriptRoot 'run-android.ps1') @runArgs
if (-not $?) { throw 'Android app launch failed after installation.' }
