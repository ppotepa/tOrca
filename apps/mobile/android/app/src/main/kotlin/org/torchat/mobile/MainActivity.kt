...
import org.torchat.transport.DirectoryEntry
...
class MainActivity : FlutterActivity() {
    ...
    private fun handle(call: MethodCall, result: MethodChannel.Result) {
        ...
        when (call.method) {
            ...
            "refreshInvite" -> result.success(TorChatForegroundService.activeController?.contactInvitePayload())
            ...
            "searchContacts" -> runAsync(result) {
                TorChatForegroundService.activeController?.searchContacts(call.argument<String>("query").orEmpty())?.map { it.toMap() }
            ...
            "startConversation" -> runAsync(result) {
                val contact = call.argument<Map<String, Any?>> ("contact")?.toContact() ?: error("invalid contact")
                TorChatForegroundService.activeController?.startConversation(contact) ?: error("runtime not connected")
                null
            }
            "startConversationFromInvite" -> runAsync(result) {
                TorChatForegroundService.activeController?.startConversation(call.argument<String>("invite").orEmpty())
                    ?: error("runtime not connected")
                TorChatForegroundService.activeController?.activeConversationId()
            }
...
private fun DirectoryEntry.toMap() = mapOf("installationId" to installationId, "nickname" to nickname, "publicKey" to publicKey, "fingerprint" to fingerprint)
private fun LocalContact.toMap() = mapOf("installationId" to installationId, "nickname" to nickname, "publicKey" to publicKey, "fingerprint" to fingerprint, "dev" to devFixture)
...
private fun Map<String, Any?>.toContact() = LocalContact(
    installationId = get("installationId") as String,
    nickname = get("nickname") as String,
    publicKey = get("publicKey") as String,
    fingerprint = get("fingerprint") as String,
    devFixture = get("dev") as? String,
)
