...
/// Durable operation persistence must be implemented by the active adapter.
///
/// Unlike the remaining transitional capabilities, this trait deliberately has
/// no blanket `RuntimeStorage` implementation. Operation lifecycle changes must
/// participate in the same transaction as the workflow mutation.
