...
impl ClientEngineActor {
    pub(super) fn with_runtime<R>(
        ...
        op: impl FnOnce(
            &mut ClientRuntime<
                ...
                SharedRuntimeClock,
            >,
        ) -> torchat_runtime::RuntimeResult<R>,
    ) -> EngineResult<(R, Vec<torchat_runtime::RuntimeEvent>)> {
    ...
    fn with_runtime_internal<R>(
            ...
            for conversation_id in &conversation_ids {
                if let Some(record) = self.database.conversation_mls_checkpoint(conversation_id)? {
                    ...
                    anchor.record_checkpoint(
                        conversation_id,
                        &torchat_crypto::anti_rollback::AnchoredMlsCheckpoint {
        ...
        if projection_changed && let Ok((store_id, revision)) = self.database.projection_head() {
            ...
            events.push(torchat_runtime::RuntimeEvent::ProjectionChanged {
                store_id,
