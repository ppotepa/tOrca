SELECT c.installation_id, c.nickname, c.public_key, c.fingerprint,
       ...
       c.transport_policy,
       CASE WHEN p.contact_installation_id IS NULL THEN 0 ELSE 1 END AS has_peer_endpoint,
       CASE WHEN EXISTS (
       ...
       CASE WHEN p.last_connected_at IS NOT NULL
                 AND p.last_connected_at >= (unixepoch() - 120)
            THEN 1 ELSE 0 END AS has_recent_peer_connection,
       p.last_connected_at,
       c.last_seen_at
FROM contacts c
